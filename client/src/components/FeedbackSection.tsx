...
import { MessageSquare, Star, Send, CheckCircle2, AlertCircle, FileSpreadsheet, HelpCircle, Copy, Check } from 'lucide-react';
...
function doPost(e) {
  ...
  return (
    <section id="feedback" className="py-20 bg-white border-t border-uday-peach/30">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Header */}
        <div className="text-center space-y-3 mb-12">
          <div className="inline-flex items-center gap-2 bg-uday-sage/15 text-uday-forest font-bold text-xs px-3.5 py-1 rounded-full uppercase tracking-wider">
          ...
          <h2 className="font-serif text-3xl sm:text-4xl font-black text-uday-midnight">
            Share Your Thoughts with <span className="text-uday-crimson">Team Uday</span>
          </h2>
          
          <p className="text-sm text-uday-midnight/70 max-w-xl mx-auto leading-relaxed">
            Your feedback, critiques, and ideas shape future editions. All responses and email addresses are logged directly into our centralized editorial Google Sheet.
          </p>
        </div>

        {/* Form Container */}
        <div className="bg-[#FAF7F2] rounded-3xl border border-uday-peach/60 p-6 sm:p-10 shadow-lg relative">
          
          {/* Top Integration Status Banner */}
          <div className="flex items-center justify-between bg-white border border-uday-peach/40 rounded-2xl p-3 px-4 mb-6 text-xs text-uday-midnight/80">
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse" />
              <span className="font-semibold">Google Sheets Sync Enabled</span>
            </div>
            <button
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-uday-peach/50 rounded-xl px-4 py-3 text-sm text-uday-midnight focus:outline-none focus:border-uday-crimson"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-uday-midnight mb-1 uppercase tracking-wider">
                    Email Address * (Recorded to Google Sheets)
                  </label>
                  <input
                    type="email"
                    required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  <label className="block text-xs font-bold text-uday-midnight mb-1 uppercase tracking-wider">
                    Roll No / Department / Affiliation (Optional)
                  </label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-uday-midnight mb-2 uppercase tracking-wider">
                  Rating of Uday Magazine & Experience
                </label>
                <div className="flex items-center gap-2">
                  {[1, 2, 3, 4, 5].map(star => (
                    <button
              ...
              <div>
                ...
                <textarea
                  required
                  rows={4}
                  placeholder="Share what you liked, what can be improved, or what topics you would love to read in the next edition..."
                  value={message}
                  onChange={e => setMessage(e.target.value)}
                  className="w-full bg-white border border-uday-peach/50 rounded-xl p-4 text-sm text-uday-midnight focus:outline-none focus:border-uday-crimson"
                />
              </div>
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  <span>{submitting ? 'Syncing to Google Sheets...' : 'Submit Feedback to Google Sheets'}</span>
                </button>
              </div>
            </form>
          )}
        ...
        {showSheetsGuide && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn">
            <div className="bg-white rounded-3xl border border-uday-peach p-6 sm:p-8 max-w-2xl w-full relative shadow-2xl animate-scaleUp max-h-[85vh] overflow-y-auto">
              ...
              <p className="text-xs sm:text-sm text-uday-midnight/70 leading-relaxed mb-4">
                Follow these 3 quick steps to stream student emails and responses directly into your institute Google Sheet:
              </p>
